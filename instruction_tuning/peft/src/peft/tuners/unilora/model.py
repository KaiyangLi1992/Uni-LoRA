...
from peft.utils import TRANSFORMERS_MODELS_TO_UNILORA_TARGET_MODULES_MAPPING, ModulesToSaveWrapper, _get_submodules
...
class UniLoRAModel(BaseTuner):
    ...
    Args:
        ...
        config ([`UniLoRAConfig`]): The configuration of the VBLoRA model.
    ...
    **Attributes**:
        - **model** ([`~transformers.PreTrainedModel`]) -- The model to be adapted.
        - **peft_config** ([`UniLoRAConfig`]): The configuration of the UniLoRAConfig model.
    """
    ...
    def _pre_injection_hook(self, model: nn.Module, config: UniLoRAConfig, adapter_name: str) -> None:
        self.unilora_vector_bank = nn.ParameterDict({})

    def _check_new_adapter_config(self, config: UniLoRAConfig) -> None:
        """
    ...
    def _create_and_replace(
        ...
        self._init_unilora_vector_bank(unilora_config, adapter_name)
        # TODO: add quantization support

        if isinstance(target, Linear):
            target.update_layer(
                adapter_name=adapter_name,
                unilora_vector_bank=self.unilora_vector_bank,
                r=unilora_config.r,
                topk=unilora_config.topk,
                num_vectors=unilora_config.num_vectors,
                vector_length=unilora_config.vector_length,
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, UniLoRALayer) and hasattr(m, "bias") and m.bias is not None:
                        m.bias.requires_grad = True
            else:
                raise NotImplementedError(f"Requested bias: {bias}, is not implemented.")

    @staticmethod
    def _create_new_module(unilora_config, unilora_vector_bank, adapter_name, target, **kwargs):
        ...
        new_module = Linear(
            base_layer=target,
            unilora_vector_bank=unilora_vector_bank,
            adapter_name=adapter_name,
            r=unilora_config.r,
            num_vectors=unilora_config.num_vectors,
            vector_length=unilora_config.vector_length,
    ...
    def _prepare_adapter_config(peft_config, model_config):
        if peft_config.target_modules is None:
            if model_config["model_type"] not in TRANSFORMERS_MODELS_TO_UNILORA_TARGET_MODULES_MAPPING:
                raise ValueError("Please specify `target_modules` in `peft_config`")
            peft_config.target_modules = set(
                TRANSFORMERS_MODELS_TO_UNILORA_TARGET_MODULES_MAPPING[model_config["model_type"]]
            )
        return peft_config
    ...
    def delete_adapter(self, adapter_name: str) -> None:
        ...
        for key in key_list:
            ...
            if isinstance(target, UniLoRALayer):
    ...
    def get_nb_savable_parameters(self, adapter="default") -> tuple[int, int]:
        ...
        for name, param in self.named_parameters():
            if "unilora_vector_bank" in name:
                vector_bank_params += param.numel()
            elif "unilora_logits" in name:
                other_params += param.numel()
            elif "unilora_norm" in name:
                other_params += param.numel()
       
            unilora_params = vector_bank_params 
